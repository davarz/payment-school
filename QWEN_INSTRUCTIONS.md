# Qwen Instructions for Payment School
...
- **Custom Blade Components**: Located in `resources/views/components/`
- **Layouts**: Modern layouts in `resources/views/layouts/` (app, guest, admin-sidebar, siswa-navbar, top-navbar, footer)
- **Reusable Elements**: Cards, stats cards, alerts, badges, form components with consistent styling
- **Responsive Design**: Mobile-first approach with Tailwind responsive classes

### Styling & Design System
- **Color Palette**: Primary blue (#3B82F6), success green (#10B981), warning orange (#F59E0B), danger red (#EF4444)
- **Typography**: Consistent font sizes and weights throughout
- **Spacing**: Standardized padding/margin system using Tailwind spacing utilities
- **Component Props**: Type-safe props for all custom components

## File Structure Notes

### Important Directories
- `app/Http/Controllers/` - Contains role-specific controllers (Admin, Siswa directories)
- `resources/views/` - Contains refactored layouts and components
- `database/migrations/` - Contains all database schema definitions
- `app/Models/` - Contains all Eloquent models with relationships
- `app/Observers/` - Contains model observers for automated actions
- `app/Services/` - Contains business logic services

### Critical Files
- `routes/web.php` - Main application routing with role-based middleware
- `app/Providers/AppServiceProvider.php` - Application service provider with observer registration
...
- `resources/views/layouts/guest.blade.php` - Guest layout (fixed $slot issue)

## Special Considerations

### When Editing Files
1. **Always check existing observers** when modifying model logic
2. **Maintain consistency** with the refactored component system
3. **Follow the existing naming conventions** (Indonesian for DB, English for code)
4. **Respect role-based access control** in any new routes/features
5. **Consider mobile responsiveness** when modifying views
6. **Follow the existing error handling patterns** for consistency
7. **Update cache appropriately** when making data changes
8. **Maintain the existing logging patterns** for debugging and security

### Security Considerations
- **Rate limiting** for password reset functionality
- **Role-based access** to all routes
- **Input validation** in all forms and requests
- **File upload validation** for payment proofs
- **SQL injection prevention** through Eloquent usage

This file should be referenced whenever making changes to ensure consistency with the existing architecture and patterns.