...
<div class="space-y-6">
    ...
    <x-page-header
        ...
        subtitle="Perbarui informasi kategori pembayaran yang telah ada"
        icon="fa-edit"
    />
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-6 bg-gradient-to-r from-blue-50 to-blue-100 border-b border-gray-200">
            <div class="flex items-start space-x-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-blue-800 mt-1">Perbarui informasi kategori pembayaran dengan benar</p>
                </div>
            </div>
        </div>
        ...
        <form action="{{ route('admin.kategori.update', $kategori->id) }}" method="POST" class="p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="nama_kategori" required value="{{ old('nama_kategori', $kategori->nama_kategori) }}"
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="3"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
                    placeholder="Deskripsi kategori pembayaran (opsional)">{{ old('deskripsi', $kategori->deskripsi) }}</textarea>
                @error('deskripsi')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" id="jumlah_formatted" required value="{{ old('jumlah', $kategori->jumlah) ? number_format(old('jumlah', $kategori->jumlah), 0, ',', '.') : '' }}"
                        ...
                        placeholder="0">
                    <input type="hidden" name="jumlah" id="jumlah_hidden" value="{{ old('jumlah', $kategori->jumlah) }}">
                </div>
                @error('jumlah')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="frekuensi" required
                        ...
                        <option value="bulanan" {{ old('frekuensi', $kategori->frekuensi) === 'bulanan' ? 'selected' : '' }}>Bulanan</option>
                        <option value="semester" {{ old('frekuensi', $kategori->frekuensi) === 'semester' ? 'selected' : '' }}>Semester</option>
                        <option value="tahunan" {{ old('frekuensi', $kategori->frekuensi) === 'tahunan' ? 'selected' : '' }}>Tahunan</option>
                    </select>
                ...
                <div>
                    ...
                    <div class="flex items-center pt-2">
                        <input type="checkbox" name="auto_generate" id="auto_generate" value="1"
                            {{ old('auto_generate', $kategori->auto_generate) ? 'checked' : '' }}
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="tahun_ajaran" required value="{{ old('tahun_ajaran', $kategori->tahun_ajaran) }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
                ...
                <div>
                    ...
                    <select name="semester" required
                        ...
                        <option value="ganjil" {{ old('semester', $kategori->semester) === 'ganjil' ? 'selected' : (now()->month >= 7 ? 'selected' : '') }}>Ganjil</option>
                        <option value="genap" {{ old('semester', $kategori->semester) === 'genap' ? 'selected' : (now()->month < 7 ? 'selected' : '') }}>Genap</option>
                    </select>
                    @error('semester')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <select name="status" required
                    ...
                    <option value="inactive" {{ old('status', $kategori->status) === 'inactive' ? 'selected' : '' }}>Nonaktif</option>
                </select>
                @error('status')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
